...
void make_enc_struct_expr(PhysicalExpr&      physical_expr,
	...
	physical_expr.operators.emplace_back(make_shared<enc_struct_opr>(column, column_descriptor));
}

/*--------------------------------------------------------------------------------------------------------------------*\
 * make_enc_list_expr
\*--------------------------------------------------------------------------------------------------------------------*/
void make_enc_list_expr(PhysicalExpr&      physical_expr,
                          const col_pt&      column,
                          ColumnDescriptorT& column_descriptor,
                          InterpreterState&  state) {

	physical_expr.operators.emplace_back(make_shared<enc_list_opr>(column, column_descriptor));
}
...
sp<PhysicalExpr>
Interpreter::Encoding::Interpret(ColumnDescriptorT& column_descriptor, const col_pt& column, InterpreterState& state) {
	// return
	...
	for (auto& [operator_tokens, operand_tokens] = *column_descriptor.encoding_rpn;
		...
		case EXP_UNCOMPRESSED_I64: {
			make_enc_uncompressed_expr<i64_pt>(*physical_expr, column, column_descriptor, state);
			break;
		}
		case EXP_UNCOMPRESSED_I32: {
			make_enc_uncompressed_expr<i32_pt>(*physical_expr, column, column_descriptor, state);
			break;
		}
		case EXP_UNCOMPRESSED_I16: {
			make_enc_uncompressed_expr<i16_pt>(*physical_expr, column, column_descriptor, state);
			break;
		}
		case EXP_UNCOMPRESSED_U08: {
			make_enc_uncompressed_expr<u08_pt>(*physical_expr, column, column_descriptor, state);
			break;
		}
		case EXP_FFOR_I64: {
			make_enc_ffor_expr<i64_pt>(*physical_expr, column, column_descriptor, state);
			break;
		}
		case EXP_FFOR_I32: {
			make_enc_ffor_expr<i32_pt>(*physical_expr, column, column_descriptor, state);
			break;
		}
		case EXP_FFOR_I16: {
			make_enc_ffor_expr<i16_pt>(*physical_expr, column, column_descriptor, state);
			break;
		}
		case EXP_FFOR_I08: {
			make_enc_ffor_expr<i08_pt>(*physical_expr, column, column_descriptor, state);
			break;
		}
		case EXP_FFOR_U08: {
			make_enc_ffor_expr<u08_pt>(*physical_expr, column, column_descriptor, state);
			break;
		}
		case EXP_UNCOMPRESSED_DBL: {
			make_enc_uncompressed_expr<dbl_pt>(*physical_expr, column, column_descriptor, state);
			break;
		}
		case EXP_UNCOMPRESSED_FLT: {
			make_enc_uncompressed_expr<flt_pt>(*physical_expr, column, column_descriptor, state);
			break;
		}
		case EXP_UNCOMPRESSED_I08: {
			make_enc_uncompressed_expr<i08_pt>(*physical_expr, column, column_descriptor, state);
			break;
		}
		case EXP_UNCOMPRESSED_STR: {
			make_fls_str_uncompressed_expr(*physical_expr, column, column_descriptor, state);
			break;
		}
		...
		case EXP_STRUCT: {
			make_enc_struct_expr(*physical_expr, column, column_descriptor, state);
			break;
		}
		case EXP_LIST: {
			make_enc_list_expr(*physical_expr, column, column_descriptor, state);
			break;
		}
		case EXP_DICT_I64_FFOR_U32: {
			make_enc_dict_ffor_expr<i64_pt, u32_pt>(*physical_expr, column, column_descriptor, state);
			break;
		}
		case EXP_DICT_I64_FFOR_U16: {
			make_enc_dict_ffor_expr<i64_pt, u16_pt>(*physical_expr, column, column_descriptor, state);
			break;
		}
		case EXP_DICT_I64_FFOR_U08: {
			make_enc_dict_ffor_expr<i64_pt, u08_pt>(*physical_expr, column, column_descriptor, state);
			break;
		}
		case EXP_DICT_I32_FFOR_U32: {
			make_enc_dict_ffor_expr<i32_pt, u32_pt>(*physical_expr, column, column_descriptor, state);
			break;
		}
		case EXP_DICT_I32_FFOR_U16: {
			make_enc_dict_ffor_expr<i32_pt, u16_pt>(*physical_expr, column, column_descriptor, state);
			break;
		}
		case EXP_DICT_I32_FFOR_U08: {
			make_enc_dict_ffor_expr<i32_pt, u08_pt>(*physical_expr, column, column_descriptor, state);
			break;
		}
		case EXP_DICT_I16_FFOR_U16: {
			make_enc_dict_ffor_expr<i16_pt, u16_pt>(*physical_expr, column, column_descriptor, state);
			break;
		}
		case EXP_DICT_DBL_FFOR_U32: {
			make_enc_dict_ffor_expr<dbl_pt, u32_pt>(*physical_expr, column, column_descriptor, state);
			break;
		}
		case EXP_DICT_I16_FFOR_U08: {
			make_enc_dict_ffor_expr<i16_pt, u08_pt>(*physical_expr, column, column_descriptor, state);
			break;
		}
		case EXP_DICT_I08_FFOR_U08: {
			make_enc_dict_ffor_expr<i08_pt, u08_pt>(*physical_expr, column, column_descriptor, state);
			break;
		}
		case EXP_DICT_DBL_FFOR_U16: {
			make_enc_dict_ffor_expr<dbl_pt, u16_pt>(*physical_expr, column, column_descriptor, state);
			break;
		}
		case EXP_DICT_FLT_FFOR_U16: {
			make_enc_dict_ffor_expr<flt_pt, u16_pt>(*physical_expr, column, column_descriptor, state);
			break;
		}
		case EXP_DICT_DBL_FFOR_U08: {
			make_enc_dict_ffor_expr<dbl_pt, u08_pt>(*physical_expr, column, column_descriptor, state);
			break;
		}
		case EXP_DICT_FLT_FFOR_U08: {
			make_enc_dict_ffor_expr<flt_pt, u08_pt>(*physical_expr, column, column_descriptor, state);
			break;
		}
		case EXP_DICT_I64_U08:
		case EXP_DICT_I64_U16:
		case EXP_DICT_I64_U32: {
			make_enc_dict_expr<i64_pt>(*physical_expr, column, column_descriptor, state);
			break;
		}
		case EXP_DICT_I32_U08:
		case EXP_DICT_I32_U16:
		case EXP_DICT_I32_U32: {
			make_enc_dict_expr<i32_pt>(*physical_expr, column, column_descriptor, state);
			break;
		}
		case EXP_DICT_I16_U08:
		case EXP_DICT_I16_U16: {
			make_enc_dict_expr<i16_pt>(*physical_expr, column, column_descriptor, state);
			break;
		}
		case EXP_DICT_I08_U08: {
			make_enc_dict_expr<i08_pt>(*physical_expr, column, column_descriptor, state);
			break;
		}
		case EXP_DICT_U08_U08: {
			make_enc_dict_expr<u08_pt>(*physical_expr, column, column_descriptor, state);
			break;
		}
		case EXP_DICT_FLT_U08: {
			make_enc_dict_expr<flt_pt>(*physical_expr, column, column_descriptor, state);
			break;
		}
		case EXP_DICT_DBL_U08:
		case EXP_DICT_DBL_U16:
		case EXP_DICT_DBL_U32: {
			make_enc_dict_expr<dbl_pt>(*physical_expr, column, column_descriptor, state);
			break;
		}
		case EXP_DICT_STR_U08:
		case EXP_DICT_STR_U16:
		case EXP_DICT_STR_U32: {
			make_enc_dict_expr<fls_string_t>(*physical_expr, column, column_descriptor, state);
			break;
		}
		case EXP_ALP_DBL: {
			make_enc_alp_expr<dbl_pt>(*physical_expr, column, column_descriptor, state);
			break;
		}
		case EXP_ALP_FLT: {
			make_enc_alp_expr<flt_pt>(*physical_expr, column, column_descriptor, state);
			break;
		}
		case EXP_GALP_FLT: {
			make_enc_galp_expr<flt_pt>(*physical_expr, column, column_descriptor, state);
			break;
		}
		case EXP_GALP_DBL: {
			make_enc_galp_expr<dbl_pt>(*physical_expr, column, column_descriptor, state);
			break;
		}
		case EXP_ALP_RD_DBL: {
			make_enc_alp_rd_expr<dbl_pt>(*physical_expr, column, column_descriptor, state);
			break;
		}
		case EXP_ALP_RD_FLT: {
			make_enc_alp_rd_expr<flt_pt>(*physical_expr, column, column_descriptor, state);
			break;
		}
		case EXP_FSST: {
			make_fsst_expr<enc_fsst_opr>(*physical_expr, column, column_descriptor, state);
			break;
		}
		case EXP_FSST12: {
			make_fsst_expr<enc_fsst12_opr>(*physical_expr, column, column_descriptor, state);
			break;
		}
		case EXP_DICT_STR_FFOR_U32: {
			make_enc_dict_ffor_expr<fls_string_t, u32_pt>(*physical_expr, column, column_descriptor, state);
			break;
		}
		case EXP_DICT_STR_FFOR_U16: {
			make_enc_dict_ffor_expr<fls_string_t, u16_pt>(*physical_expr, column, column_descriptor, state);
			break;
		}
		case EXP_DICT_STR_FFOR_U08: {
			make_enc_dict_ffor_expr<fls_string_t, u08_pt>(*physical_expr, column, column_descriptor, state);
			break;
		}
		case EXP_FSST_DICT_STR_FFOR_U32: {
			make_enc_fsst_dict_ffor_expr<enc_fsst_dict_opr, fls_string_t, u32_pt>(
			    *physical_expr, column, column_descriptor, state);
			break;
		}
		...
		case EXP_RLE_DBL_U16: {
			make_enc_rle_expr<dbl_pt, u16_pt>(*physical_expr, column, column_descriptor, state);
			break;
		}
		case EXP_RLE_FLT_U16: {
			make_enc_rle_expr<flt_pt, u16_pt>(*physical_expr, column, column_descriptor, state);
			break;
		}
		case EXP_RLE_I64_U16: {
			make_enc_rle_expr<i64_pt, u16_pt>(*physical_expr, column, column_descriptor, state);
			break;
		}
		case EXP_RLE_I32_U16: {
			make_enc_rle_expr<i32_pt, u16_pt>(*physical_expr, column, column_descriptor, state);
			break;
		}
		case EXP_RLE_I16_U16: {
			make_enc_rle_expr<i16_pt, u16_pt>(*physical_expr, column, column_descriptor, state);
			break;
		}
		case EXP_RLE_I08_U16: {
			make_enc_rle_expr<i08_pt, u16_pt>(*physical_expr, column, column_descriptor, state);
			break;
		}
		case EXP_RLE_U08_U16: {
			make_enc_rle_expr<u08_pt, u16_pt>(*physical_expr, column, column_descriptor, state);
			break;
		}
		case EXP_RLE_STR_U16: {
			make_enc_rle_expr<fls_string_t, u16_pt>(*physical_expr, column, column_descriptor, state);
			break;
		}
		case EXP_FSST_DELTA: {
			make_fsst_delta_expr<enc_fsst_opr>(*physical_expr, column, column_descriptor, state);
			break;
		}
		case EXP_FSST12_DELTA: {
			make_fsst_delta_expr<enc_fsst12_opr>(*physical_expr, column, column_descriptor, state);
			break;
		}
		case EXP_DELTA_I64: {
			make_enc_delta_expr<i64_pt>(*physical_expr, column, column_descriptor, state);
			break;
		}
		case EXP_DELTA_I32: {
			make_enc_delta_expr<i32_pt>(*physical_expr, column, column_descriptor, state);
			break;
		}
		case EXP_DELTA_I16: {
			make_enc_delta_expr<i16_pt>(*physical_expr, column, column_descriptor, state);
			break;
		}
		case EXP_DELTA_I08: {
			make_enc_delta_expr<i08_pt>(*physical_expr, column, column_descriptor, state);
			break;
		}
		case EXP_DELTA_U08: {
			make_enc_delta_expr<u08_pt>(*physical_expr, column, column_descriptor, state);
			break;
		}
		case EXP_FFOR_SLPATCH_I64: {
			make_enc_ffor_slpatch_expr<i64_pt>(*physical_expr, column, column_descriptor, state);
			break;
		}
		case EXP_FFOR_SLPATCH_I32: {
			make_enc_ffor_slpatch_expr<i32_pt>(*physical_expr, column, column_descriptor, state);
			break;
		}
		case EXP_FFOR_SLPATCH_I16: {
			make_enc_ffor_slpatch_expr<i16_pt>(*physical_expr, column, column_descriptor, state);
			break;
		}
		case EXP_FFOR_SLPATCH_I08: {
			make_enc_ffor_slpatch_expr<i08_pt>(*physical_expr, column, column_descriptor, state);
			break;
		}
		case EXP_FFOR_SLPATCH_U08: {
			make_enc_ffor_slpatch_expr<u08_pt>(*physical_expr, column, column_descriptor, state);
			break;
		}
		case EXP_DICT_I64_FFOR_SLPATCH_U16: {
			make_enc_dict_ffor_slpatch_expr<i64_pt, u16_pt>(*physical_expr, column, column_descriptor, state);
			break;
		}
		case EXP_DICT_I64_FFOR_SLPATCH_U32: {
			make_enc_dict_ffor_slpatch_expr<i64_pt, u32_pt>(*physical_expr, column, column_descriptor, state);
			break;
		}
		case EXP_DICT_I32_FFOR_SLPATCH_U32: {
			make_enc_dict_ffor_slpatch_expr<i32_pt, u32_pt>(*physical_expr, column, column_descriptor, state);
			break;
		}
		case EXP_DICT_I32_FFOR_SLPATCH_U16: {
			make_enc_dict_ffor_slpatch_expr<i32_pt, u16_pt>(*physical_expr, column, column_descriptor, state);
			break;
		}
		case EXP_DICT_I32_FFOR_SLPATCH_U08: {
			make_enc_dict_ffor_slpatch_expr<i32_pt, u08_pt>(*physical_expr, column, column_descriptor, state);
			break;
		}
		case EXP_DICT_DBL_FFOR_SLPATCH_U16: {
			make_enc_dict_ffor_slpatch_expr<dbl_pt, u16_pt>(*physical_expr, column, column_descriptor, state);
			break;
		}
		case EXP_DICT_STR_FFOR_SLPATCH_U32: {
			make_enc_dict_ffor_slpatch_expr<fls_string_t, u32_pt>(*physical_expr, column, column_descriptor, state);
			break;
		}
		case EXP_DICT_STR_FFOR_SLPATCH_U16: {
			make_enc_dict_ffor_slpatch_expr<fls_string_t, u16_pt>(*physical_expr, column, column_descriptor, state);
			break;
		}
		case EXP_DICT_STR_FFOR_SLPATCH_U08: {
			make_enc_dict_ffor_slpatch_expr<fls_string_t, u08_pt>(*physical_expr, column, column_descriptor, state);
			break;
		}
		case EXP_DICT_I16_FFOR_SLPATCH_U08: {
			make_enc_dict_ffor_slpatch_expr<i16_pt, u08_pt>(*physical_expr, column, column_descriptor, state);
			break;
		}
		case EXP_DICT_I08_FFOR_SLPATCH_U08: {
			make_enc_dict_ffor_slpatch_expr<i08_pt, u08_pt>(*physical_expr, column, column_descriptor, state);
			break;
		}
		case EXP_DICT_I16_FFOR_SLPATCH_U16: {
			make_enc_dict_ffor_slpatch_expr<i16_pt, u16_pt>(*physical_expr, column, column_descriptor, state);
			break;
		}
		case EXP_DICT_I64_FFOR_SLPATCH_U08: {
			make_enc_dict_ffor_slpatch_expr<i64_pt, u08_pt>(*physical_expr, column, column_descriptor, state);
			break;
		}
		case EXP_DICT_DBL_FFOR_SLPATCH_U08: {
			make_enc_dict_ffor_slpatch_expr<dbl_pt, u08_pt>(*physical_expr, column, column_descriptor, state);
			break;
		}
		case EXP_DICT_FLT_FFOR_SLPATCH_U08: {
			make_enc_dict_ffor_slpatch_expr<flt_pt, u08_pt>(*physical_expr, column, column_descriptor, state);
			break;
		}
		case EXP_DICT_FLT_FFOR_SLPATCH_U16: {
			make_enc_dict_ffor_slpatch_expr<flt_pt, u16_pt>(*physical_expr, column, column_descriptor, state);
			break;
		}
		case EXP_DICT_DBL_FFOR_SLPATCH_U32: {
			make_enc_dict_ffor_slpatch_expr<dbl_pt, u32_pt>(*physical_expr, column, column_descriptor, state);
			break;
		}
		...
		case EXP_NULL_DBL: {
			make_enc_null_expr<dbl_pt>(*physical_expr, column, column_descriptor, state);
			break;
		}
		case EXP_NULL_FLT: {
			make_enc_null_expr<flt_pt>(*physical_expr, column, column_descriptor, state);
			break;
		}
		case EXP_NULL_I16: {
			make_enc_null_expr<i16_pt>(*physical_expr, column, column_descriptor, state);
			break;
		}
		case EXP_NULL_I32: {
			make_enc_null_expr<i32_pt>(*physical_expr, column, column_descriptor, state);
			break;
		}
		case EXP_RLE_I64_SLPATCH_U16: {
			make_enc_rle_slpatch_expr<i64_pt, u16_pt>(*physical_expr, column, column_descriptor, state);
			break;
		}
		case EXP_RLE_I32_SLPATCH_U16: {
			make_enc_rle_slpatch_expr<i32_pt, u16_pt>(*physical_expr, column, column_descriptor, state);
			break;
		}
		case EXP_RLE_I16_SLPATCH_U16: {
			make_enc_rle_slpatch_expr<i16_pt, u16_pt>(*physical_expr, column, column_descriptor, state);
			break;
		}
		case EXP_RLE_I08_SLPATCH_U16: {
			make_enc_rle_slpatch_expr<i08_pt, u16_pt>(*physical_expr, column, column_descriptor, state);
			break;
		}
		case EXP_RLE_U08_SLPATCH_U16: {
			make_enc_rle_slpatch_expr<u08_pt, u16_pt>(*physical_expr, column, column_descriptor, state);
			break;
		}
		case EXP_RLE_STR_SLPATCH_U16: {
			make_enc_rle_slpatch_expr<fls_string_t, u16_pt>(*physical_expr, column, column_descriptor, state);
			break;
		}
		case EXP_RLE_DBL_SLPATCH_U16: {
			make_enc_rle_slpatch_expr<dbl_pt, u16_pt>(*physical_expr, column, column_descriptor, state);
			break;
		}
		case EXP_RLE_FLT_SLPATCH_U16: {
			make_enc_rle_slpatch_expr<flt_pt, u16_pt>(*physical_expr, column, column_descriptor, state);
			break;
		}
		case EXP_FSST_DICT_STR_U32:
		case EXP_FSST_DICT_STR_U16:
		case EXP_FSST_DICT_STR_U08: {
			make_enc_fsst_dict_expr<enc_fsst_dict_opr>(*physical_expr, column, column_descriptor, state);
			break;
		}
		case EXP_FSST_DELTA_SLPATCH: {
			make_enc_fsst_delta_slpatch_expr<enc_fsst_opr>(*physical_expr, column, column_descriptor, state);
			break;
		}
		case EXP_FSST12_DELTA_SLPATCH: {
			make_enc_fsst_delta_slpatch_expr<enc_fsst12_opr>(*physical_expr, column, column_descriptor, state);
			break;
		}
		...
		case EXP_FSST12_DICT_STR_U08: {
			make_enc_fsst_dict_expr<enc_fsst12_dict_opr>(*physical_expr, column, column_descriptor, state);
			break;
		}
		...
		case EXP_FREQUENCY_DBL: {
			make_enc_frequency_expr<dbl_pt>(*physical_expr, column, column_descriptor, state);
			break;
		}
		case EXP_FREQUENCY_FLT: {
			make_enc_frequency_expr<flt_pt>(*physical_expr, column, column_descriptor, state);
			break;
		}
		case EXP_FREQUENCY_I08: {
			make_enc_frequency_expr<i08_pt>(*physical_expr, column, column_descriptor, state);
			break;
		}
		case EXP_FREQUENCY_U08: {
			make_enc_frequency_expr<u08_pt>(*physical_expr, column, column_descriptor, state);
			break;
		}
		case EXP_FREQUENCY_I16: {
			make_enc_frequency_expr<i16_pt>(*physical_expr, column, column_descriptor, state);
			break;
		}
		case EXP_FREQUENCY_I32: {
			make_enc_frequency_expr<i32_pt>(*physical_expr, column, column_descriptor, state);
			break;
		}
		case EXP_FREQUENCY_I64: {
			make_enc_frequency_expr<i64_pt>(*physical_expr, column, column_descriptor, state);
			break;
		}
		case EXP_FREQUENCY_STR: {
			make_enc_frequency_str_opr(*physical_expr, column, column_descriptor, state);
			break;
		}
		case EXP_CROSS_RLE_I08: {
			make_enc_cross_rle_opr<i08_pt>(*physical_expr, column, column_descriptor, state);
			break;
		}
		case EXP_CROSS_RLE_U08: {
			make_enc_cross_rle_opr<u08_pt>(*physical_expr, column, column_descriptor, state);
			break;
		}
		case EXP_CROSS_RLE_I16: {
			make_enc_cross_rle_opr<i16_pt>(*physical_expr, column, column_descriptor, state);
			break;
		}
		case EXP_CROSS_RLE_I32: {
			make_enc_cross_rle_opr<i32_pt>(*physical_expr, column, column_descriptor, state);
			break;
		}
		case EXP_CROSS_RLE_I64: {
			make_enc_cross_rle_opr<i64_pt>(*physical_expr, column, column_descriptor, state);
			break;
		}
		case EXP_CROSS_RLE_DBL: {
			make_enc_cross_rle_opr<dbl_pt>(*physical_expr, column, column_descriptor, state);
			break;
		}
		case EXP_CROSS_RLE_FLT: {
			make_enc_cross_rle_opr<flt_pt>(*physical_expr, column, column_descriptor, state);
			break;
		}
		case EXP_CROSS_RLE_STR: {
			make_enc_cross_rle_opr<fls_string_t>(*physical_expr, column, column_descriptor, state);
			break;
		}
		case EXP_VALIDITY_MASK: {
			make_enc_validitymask_expr(*physical_expr, column, column_descriptor, state);
			break;
		}
		case INVALID:
		default:
...
void Interpreter::Decoding::Interpret(const ColumnDescriptorT& column_descriptor,
	...
	for (const auto& [operator_tokens, operand_tokens] = *column_descriptor.encoding_rpn;
		...
		case EXP_DELTA_U08: {
			make_dec_delta_expr<u08_pt>(reader, physical_expr, column_view, state);
			break;
		}
		case EXP_FFOR_SLPATCH_I64: {
		...
		case EXP_RLE_U08_SLPATCH_U16: {
			make_dec_rle_slpatch_expr<u08_pt, u16_pt>(reader, physical_expr, column_view, state);
			break;
		}
		case EXP_RLE_STR_SLPATCH_U16: {
